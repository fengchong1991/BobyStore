...
namespace BabyStore.Controllers
{
    public class ProductImagesController : Controller
    {
        ...
        {
            return View(db.ProductImgaes.ToList());
        }
        ...
        public ActionResult Details(int? id)
        {
            ...
            ProductImage productImage = db.ProductImgaes.Find(id);
        ...
        public ActionResult Upload(HttpPostedFileBase[] files)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (var file in files)
                {
                    ...
                    try
                    {
                        db.ProductImgaes.Add(productToAdd);
                        db.SaveChanges();
                    }
            ...
            //        catch (Exception)
            //        {
            //            ModelState.AddModelError("FileName", "Sorry an error occurred saving the to disk, please try again");
            //        }
            //    }
            //    else
            //    {
            //        ModelState.AddModelError("FileName", "The file must be gif, png, jpeg or jpg less than 2MB in size");
            //    }
            //}
            //else
            //{
            ...
            //if (ModelState.IsValid)
            //{
            //    db.ProductImgaes.Add(new ProductImage { FileName = file.FileName });
            //    try
            //    {
            //        db.SaveChanges();
            //    }
            ...
            //        else
            //        {
            //            ModelState.AddModelError("FileName", "Sorry an error has occurred saving the database, please try again");
            //        }
            //        return View();
            //    }
            //    return RedirectToAction("Index");
        ...
        {
            ProductImage productImage = db.ProductImgaes.Find(id);
            db.ProductImgaes.Remove(productImage);
            db.SaveChanges();
            return RedirectToAction("Index");
        }
